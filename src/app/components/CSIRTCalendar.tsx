...
const [selectedDate, setSelectedDate] = useState(() => {
  ...
  return null; // para evitar que el SSR genere un valor distinto
});
...
useEffect(() => {
  ...
  const hasEvent = (date) => eventData.some(event => event.date === date);

  const getEventsForDate = (date) => eventData.filter(event => event.date === date);
        ...
        <div className="flex flex-col lg:flex-row">
          <div className="w-full lg:w-1/2 lg:border-r border-green-400/20 pr-0 lg:pr-6 pb-6 lg:pb-0">
            <div className="flex justify-between items-center mb-6">
              <button onClick={prevMonth} className="flex items-center text-green-400 hover:text-green-300">
              ...
              <button onClick={nextMonth} className="flex items-center text-green-400 hover:text-green-300">
  ...
  <div className="grid grid-cols-7 gap-2 mb-4">
    {dayNames.map((day, index) => (
      <div key={index} className="text-center text-sm text-gray-400 p-1 font-semibold">{day}</div>
    ))}
  </div>
            ...
            {selectedDate && (
              <>
                <h2 className="text-xl font-bold mb-6 text-white">
                  EVENTOS PARA {formatSelectedDate(selectedDate).toUpperCase()}
                </h2>
